...
export function TaxonomyManager({ ages, skills }: { ages: AgeGroupItem[]; skills: SkillItem[] }) {
  ...
  return (
    <div className="space-y-6">
      <section>
        <h2 className="type-section-title">{contentText(content, "taxonomy.ageTitle", "Nhóm tuổi")}</h2>
              ...
              <summary className="flex min-h-20 cursor-pointer list-none items-center justify-between gap-3 px-4 py-3 marker:hidden">
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    <h3 className="type-card-title">{age.label}</h3>
                    <span
                  ...
                  <p className="type-supporting mt-1 line-clamp-2 text-[#6f6558]">{age.description}</p>
                </div>
          ...
          {skills.map((skill) => (
            ...
            >
              <summary className="flex min-h-20 cursor-pointer list-none items-center justify-between gap-3 px-4 py-3 marker:hidden">
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    <h3 className="type-card-title truncate">{skill.title}</h3>
                    <span className="type-caption rounded-full bg-[#eef4e4] px-2.5 py-1 font-black text-[#557047]">
                  ...
                  <p className="type-supporting mt-1 line-clamp-2 text-[#6f6558]">{skill.description}</p>
                </div>
                <ChevronDown size={19} className="shrink-0 transition group-open:rotate-180" />
              </summary>
              <div className="border-t border-[#eadfc9] p-4">
        ...
        <details className="group mt-3 overflow-hidden rounded-2xl border-2 border-dashed bg-white/70">
          <summary className="flex min-h-16 cursor-pointer list-none items-center justify-between gap-3 px-4 py-3 marker:hidden">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="type-card-title">Thêm kỹ năng mới</h3>
                <p className="type-supporting mt-0.5 text-[#6f6558]">Mở biểu mẫu khi cần bổ sung kỹ năng.</p>
              </div>
            </div>
