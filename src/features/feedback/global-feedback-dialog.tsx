...
import { z } from "zod";
import { feedbackApi, type FeedbackUploadConfig } from "@/api/feedback";
...
const FeedbackImageAnnotator = dynamic(
  () =>
    import("@/features/feedback/feedback-image-annotator").then((module) => module.FeedbackImageAnnotator),
  {
    loading: () => (
      <div
        data-feedback-ui
        role="status"
        className="fixed inset-0 z-[120] grid place-items-center bg-black/55 p-5"
      >
        <div className="rounded-2xl bg-white px-5 py-4 font-bold shadow-xl">
          Đang mở công cụ đánh dấu ảnh...
        </div>
      </div>
    ),
  },
...
type GlobalFeedbackDialogProps = {
  open: boolean;
  config: FeedbackUploadConfig;
  common: ContentDictionary;
  onClose: () => void;
};

export function GlobalFeedbackDialog({ open, config, common, onClose }: GlobalFeedbackDialogProps) {
  ...
  const mutation = useMutation({
    mutationFn: (values: FormValues) => {
      const configuredLimit = config.maxAttachments ?? 0;
  ...
  }, [form, mutation.isPending, onClose]);
  ...
  }, [attachments, config.maxAttachments]);
  ...
  useEffect(() => {
    ...
    if (autoCaptureStartedRef.current || config.maxAttachments <= 0) return;
    autoCaptureStartedRef.current = true;
    const frame = window.requestAnimationFrame(() => void takeScreenshot());
    return () => window.cancelAnimationFrame(frame);
  }, [config.maxAttachments, open, takeScreenshot]);
  ...
  async function addUploads(files: File[]) {
    ...
    try {
      for (const file of files) {
        ...
        try {
          await validateImageFileForCategory(file, "feedback-attachment", config.policies);
          accepted.push(file);
        } catch (error) {
  ...
  const selectedAttachments = attachments.slice(0, config.maxAttachments);
  const policySummary = getImageUploadPolicySummary("feedback-attachment", config.policies);
