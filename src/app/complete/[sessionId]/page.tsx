...
export default async function Page({ params }: { params: Promise<{ sessionId: string }> }) {
  ...
  const summary = await getCompletionSummary(auth.user.id, sessionId);
  if (!summary) notFound();
  return (
    <ChildShell>
      <BrandHeader />
      <main className="paper-texture min-h-[calc(100vh-5rem)] px-5 pt-4 pb-8">
        <div className="relative overflow-hidden rounded-[28px]">
          <Image
            ...
            height={760}
            priority
            alt="Bé và Bống ăn mừng hoàn thành nhiệm vụ"
            className="h-[390px] w-full object-cover"
          />
          <div className="absolute inset-x-3 top-3 rounded-[24px] border-4 border-[#8d6031] bg-[#f2d28c]/95 p-4 text-center">
            <p className="text-xs font-black tracking-[.2em] text-[#50733c] uppercase">Nhiệm vụ hoàn thành</p>
        ...
        {summary.badge ? (
          <Card className="relative mx-3 -mt-6 flex items-center gap-4 p-4">
            <Image
              ...
              width={96}
              height={96}
              alt={`Huy hiệu ${summary.badge.name}`}
        ...
        <h2 className="mt-5 text-center text-xl font-black">Con đã luyện những thói quen nào?</h2>
        ...
        <Card className="mt-5 bg-[#fff3d5] p-4 text-center">
          <p className="font-bold">Mỗi lần nhìn kỹ và thử lại là một lần bộ não khỏe hơn một chút!</p>
        </Card>
        <Link
          href="/missions"
