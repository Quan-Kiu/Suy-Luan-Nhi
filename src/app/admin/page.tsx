...
export default async function Page() {
  ...
  const cards = [
    { label: "Tài khoản", value: data.users, icon: Users, href: "/admin/members" },
    { label: "Hồ sơ bé", value: data.children, icon: Users, href: "/admin/reports" },
    { label: "Phiên chơi 30 ngày", value: data.sessions30d, icon: MonitorDot, href: "/admin/reports" },
    { label: "Chờ duyệt", value: data.pendingReviews, icon: ClipboardCheck, href: "/admin/reviews" },
    { label: "Media chờ duyệt", value: data.pendingMedia, icon: ImageIcon, href: "/admin/media" },
  ];
  return (
    <div>
      <div>
        <p className="text-sm text-[#806d54]">Operational overview</p>
      ...
      <div className="mt-6 grid gap-5 lg:grid-cols-[1fr_.75fr]">
        <Card className="p-5">
          <div className="flex items-center justify-between">
            <h2 className="text-xl font-black">Trạng thái nội dung</h2>
          ...
          {!Object.keys(data.missionCounts).length ? (
            <p className="mt-4 text-sm">Chưa có dữ liệu nhiệm vụ.</p>
          ) : null}
        </Card>
        <Card className="p-5">
          <div className="flex items-center gap-2">
            <FileText size={20} />
            <h2 className="text-xl font-black">Audit gần đây</h2>
          </div>
