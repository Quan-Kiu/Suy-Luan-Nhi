...
Store one automatic feedback row per fingerprint with occurrence count, first-seen time, and last-seen time. Serialize writes with a PostgreSQL advisory transaction lock and enforce uniqueness with an index. Repeated reports refresh the latest diagnostic context and increment the counter. A resolved or dismissed fault that returns after thirty minutes reopens as new.
...
Migration `0025_automatic_feedback_deduplication.sql` fingerprints and merges existing automatic-report duplicates while leaving manual feedback untouched.
